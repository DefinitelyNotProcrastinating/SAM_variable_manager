...
// == Version: 4.0.1 "Lepton" (Refactored for External API)
// ==
...
(function () {
    ...
    const SCRIPT_VERSION = "4.0.1 'Lepton'"; // NEW: Version Name
    ...
    // a flag to pause everything.
    // for example, when chat length is 0, we pause everything until chat length is >= 1.
    var pauseFlag = 0;
            ...
            switch (curr_state) {
                case STATES.IDLE:
                    switch (event) {
                        ...
                        case event_types.GENERATION_STARTED: // GENERATION_STARTED covers both regular and swipe/regenerate flows
                        ...
                        case event_types.MESSAGE_EDITED: // An edit can alter commands, so resync
                        case event_types.CHAT_CHANGED: // Chat change always requires full sync
    ...
    const handlers = {
		...
		handleGenerationEnded: async () => {
			await unifiedEventHandler(event_types.GENERATION_ENDED);
		},
		handleMessageSwiped: () => {
			setTimeout(async () => {
				await unifiedEventHandler(event_types.MESSAGE_SWIPED);
			}, 0);
		},
		handleMessageDeleted: (message) => {
			setTimeout(async () => {
				await unifiedEventHandler(event_types.MESSAGE_DELETED, message);
			}, 0);
		},
		handleMessageEdited: () => {
			setTimeout(async () => {
				await unifiedEventHandler(event_types.MESSAGE_EDITED);
			}, 0);
		},
		handleChatChanged: () => {
			setTimeout(async () => {
				await unifiedEventHandler(event_types.CHAT_CHANGED);
			}, 10);
		},
		handleMessageSent: () => {
			setTimeout(async () => {
				await unifiedEventHandler(event_types.MESSAGE_SENT);
			}, 0);
		},
		handleGenerationStopped: () => {
			setTimeout(async () => {
				await unifiedEventHandler(event_types.GENERATION_STOPPED);
			}, 0);
		},
	};
    ...
    /**
     * Returns the current state of the SAM Finite State Machine.
     * @returns {string} The current state (e.g., "IDLE", "PROCESSING").
     */
    function sam_get_state() {
        return curr_state;
    };

    /**
     * Asynchronously returns the most current, fully reconstructed data block.
     * @returns {Promise<object>} A promise that resolves with the SAM data object.
     */
    async function sam_get_data() {
        logger.info("[External API] sam_get_data called.");
    ...
    /**
     * Sets the data block in the last AI message to a new value.
     * If a data block is present, it's replaced. If not, a new checkpoint is inserted.
     * @param {object} newData The new state object to write to the message.
     */
    async function sam_set_data(newData) {
        logger.info("[External API] sam_set_data called.");
        if (typeof newData !== 'object' || newData === null) {
            logger.error("[External API] sam_set_data requires a valid object.");
            toastr.error("SAM API: sam_set_data requires a valid object.");
            return;
        }

        // Use a lock to prevent conflicts with other operations
        if (isCheckpointing || isProcessingState || curr_state !== STATES.IDLE) {
            toastr.warning("SAM is busy. Cannot set data now. Please wait.");
            logger.warn("[External API] sam_set_data blocked: SAM is busy.");
            return;
        }
        isCheckpointing = true; // Use the checkpointing lock
        
        try {
            const lastAiIndex = await findLastAiMessageAndIndex();
            if (lastAiIndex === -1) {
                toastr.error("SAM API: Cannot set data. No AI message found.");
                logger.warn("[External API] sam_set_data failed: No AI message in chat history.");
                return;
            }
            ...
            isCheckpointing = false; // Release the lock
        }
    };
    
    /**
     * Resets the current state of the FSM to IDLE and aborts any processing cycles.
     */
    function sam_abort_cycle() {
        logger.info("[External API] sam_abort_cycle called.");
        resetCurrentState();
    };
...
(() => {
    ...
    $(() => {
        ...
        try {
            ...
            // no more debug buttons! All buttons will be displayed in plugin UI.
            /*
            try {
